...
const createImages = (id, arr, headers) =>
  Promise.all(
    arr.map(async (image, i) => {
      const variables = { image, id, primary: i === 0 };
      await makeRequest(CREATE_IMAGE_MUTATION, variables, headers);
    })
  );
...
const createAd = async (req, res, next) => {
  ...
  try {
    ...
    console.log({ data });
    console.log({ errors });
    const imagesResponse = await createImages(data.id, images, headers);

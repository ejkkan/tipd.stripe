...
const createAd = async (req, res, next) => {
  ...
  try {
    ...
    const text =
      "INSERT INTO ads(title, description, type, price, state) VALUES($1, $2, $3, $4, $5) RETURNING *";
    const values = [title, description, type, 100, "free"];
    const res = await pool.query(
      text,
      values,
      parseInt(headers["x-hasura-user-id"])
    );
    console.log("res", res);
    // console.log({ data });
